...
#include "cartographer_bindings/correlative_scan_matcher_2d.h"
...
SearchParameters::SearchParameters(const double linear_search_window,
  ...
  for (const auto& point : point_cloud) {
    max_scan_range = std::max(point.norm(), max_scan_range);
  }
...
std::vector<DiscreteScan2D> GenerateRotatedScans(
  ...
  discrete_scans.reserve(search_parameters.num_scans);

  double delta_theta = -search_parameters.num_angular_perturbations *
                       search_parameters.angular_perturbation_step_size + 
                       initial_pose_estimate.rotation().angle();
  Eigen::Vector2f initial_translation = initial_pose_estimate.translation().cast<float>();
  for (int scan_index = 0; scan_index < search_parameters.num_scans;
       ++scan_index,
           delta_theta += search_parameters.angular_perturbation_step_size) {
    discrete_scans.emplace_back();
    discrete_scans.back().reserve(point_cloud.size());
    Eigen::Matrix2f rot = Eigen::Rotation2Df(delta_theta).toRotationMatrix();
    for (const auto& point : point_cloud) {
      const Eigen::Vector2f translated_point = rot * point + initial_translation;
